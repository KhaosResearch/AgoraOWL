...
### Fallos esperados con `--include-official-examples`

`scripts/validate_shacl.py --include-official-examples` valida también los RDF oficiales de `examples/dcat-ap-es/rdf/1.0.0/` (copias sin modificar del repositorio [datosgobes/DCAT-AP-ES](https://github.com/datosgobes/DCAT-AP-ES)). Estos ficheros **fallan actualmente y es el comportamiento esperado**, no una regresión de AgoraOWL:

- **Herencia Catálogo → Dataset**: AgoraOWL declara `dcat:Catalog rdfs:subClassOf dcat:Dataset` (correcto según DCAT3) y el script valida con `inference="rdfs"`. Al fusionar la ontología con las shapes oficiales, el nodo `dcat:Catalog` del ejemplo hereda también las restricciones obligatorias de `DatasetShape` (`dcat:theme`, `dcat:distribution`, `dcat:contactPoint`, etc.), que no le corresponden como catálogo.
- **Entidades auxiliares fuera de alcance**: los propios ejemplos oficiales indican en sus comentarios internos que omiten deliberadamente bloques auxiliares (p. ej. "sin necesidad de declarar un bloque foaf:Agent específico" para el publicador, o el punto de contacto por ser "opcional"), asumiendo vocabularios externos (directorio de organismos de datos.gob.es, catálogo de licencias de la UE) que no se cargan al validar el fichero de forma aislada. Esto produce violaciones sobre `foaf:name`/`dct:type` en recursos externos que el propio ejemplo no pretende describir.
- **Inconsistencia ya presente en las shapes oficiales**: `shacl_dataset_shape.ttl` marca `dcat:contactPoint` como `sh:Violation` (obligatorio) también para `dcat:Dataset`, lo que contradice la nota del ejemplo `E_DCAT-AP-ES_minimal.rdf` que lo describe como opcional.

Por este motivo los ejemplos oficiales **no se editan** en este repositorio (perderían su valor como referencia fiel al original) y la comprobación se mantiene opcional (`--include-official-examples`), fuera de la validación por defecto y de CI.
